-- Migration script to copy dev schema to first_congregational schema
-- Modified for direct execution without psql variables

-- Create the target schema if it doesn't exist
CREATE SCHEMA IF NOT EXISTS first_congregational;
...
CREATE TABLE :"target_schema".event_types (
    id integer NOT NULL,
    name text NOT NULL,
    description text,
...
ALTER SEQUENCE :"target_schema".event_types_id_seq OWNED BY :"target_schema".event_types.id;
...
ALTER TABLE ONLY :"target_schema".event_types ALTER COLUMN id SET DEFAULT nextval(':"target_schema".event_types_id_seq'::regclass);
...
ALTER TABLE ONLY :"target_schema".participant_event_types ALTER COLUMN id SET DEFAULT nextval(':"target_schema".participant_event_types_id_seq'::regclass);
...
ALTER TABLE ONLY :"target_schema".event_types
...
CREATE INDEX idx_grp_topic_avatar_turns_topicpathid ON :"target_schema".grp_topic_avatar_turns USING btree (topicpathid);
...
ALTER TABLE ONLY :"target_schema".grp_topic_avatars
    ADD CONSTRAINT fk_grp_topic_avatars_topic_path FOREIGN KEY (topic_path_id) REFERENCES :"target_schema".topic_paths(id) ON DELETE CASCADE;
