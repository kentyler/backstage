// tests/groupConversationAvatars.test.js
...
import {
  createGroupConversationAvatar,
  getGroupConversationAvatarsByConversation,
  deleteGroupConversationAvatar
} from '../src/db/groupConversationAvatars';
...
describe('Group Conversation Avatars', () => {
  ...
  it('creates a conversation-avatar link', async () => {
    const row = await createGroupConversationAvatar(convId, TEST_AVATAR_ID);
    expect(row).toHaveProperty('group_conversation_id', convId);
  ...
  it('lists avatars by conversation', async () => {
    // link two avatars
    await createGroupConversationAvatar(convId, TEST_AVATAR_ID);
    await createGroupConversationAvatar(convId, TEST_AVATAR_ID + 1); 
    const list = await getGroupConversationAvatarsByConversation(convId);
  ...
  it('deletes a conversation-avatar link', async () => {
    await createGroupConversationAvatar(convId, TEST_AVATAR_ID);
    const ok = await deleteGroupConversationAvatar(convId, TEST_AVATAR_ID);
    expect(ok).toBe(true);
