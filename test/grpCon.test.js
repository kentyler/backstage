...
describe('Group Conversation Functions', () => {
  beforeEach(async () => {
    await pool.query(
      "DELETE FROM public.grp_cons WHERE group_id = $1 AND name LIKE 'test-%'",
      [testGroup]
    );
  ...
  afterEach(async () => {
    await pool.query('DELETE FROM public.grp_cons WHERE id = $1', [convId]);
  });
  ...
  it('creates a conversation', async () => {
    ...
    await pool.query('DELETE FROM public.grp_cons WHERE id = $1', [c.id]);
  });
