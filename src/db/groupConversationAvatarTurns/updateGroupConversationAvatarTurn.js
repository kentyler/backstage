// src/db/groupConversationAvatarTurn/updateAvatarTurn.js
...
export async function updateGroupConversationAvatarTurn(id, newText, newVector) {
  const normalized = normalizeVector(newVector);
  const vecLit     = toVectorLiteral(normalized);
  const query = `
    UPDATE public.group_conversation_avatar_turns
       SET content_text = $2,
